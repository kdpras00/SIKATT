...
<div class="content">
    ...
    <table class="table-data">
        ...
        <tr><td class="label-col">Status Perkawinan</td><td class="separator-col">:</td><td>{{ $data['marital_status'] ?? ($letter->user->marital_status ?? '-') }}</td></tr>
    ...
    <p>Dengan ini menerangkan bahwa benar yang bersangkutan pada saat ini memiliki / membuka usaha di wilayah administrasi Kelurahan Tanah Tinggi sebagai berikut:</p>

    <table class="table-data">
        <tr><td class="label-col">Nama Usaha</td><td class="separator-col">:</td><td><strong>{{ strtoupper($data['business_name'] ?? '-') }}</strong></td></tr>
        <tr><td class="label-col">Jenis Usaha</td><td class="separator-col">:</td><td>{{ $data['business_type'] ?? '-' }}</td></tr>
        <tr><td class="label-col">Alamat Usaha</td><td class="separator-col">:</td><td>{{ $data['business_address'] ?? '-' }}</td></tr>
    </table>

    @php $endDate = (clone $date)->addYear(); @endphp
    <p style="font-weight: bold;">Surat Keterangan Usaha ini berlaku selama (1 tahun) sejak dikeluarkannya surat ini Tanggal {{ $date->translatedFormat('d F Y') }} s/d {{ $endDate->translatedFormat('d F Y') }}.</p>
    <p>Demikian Surat Keterangan Usaha ini kami buat dengan sesungguhnya untuk dapat dipergunakan sebagaimana mestinya.</p>
